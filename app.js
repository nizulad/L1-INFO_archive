...
// Communicates with Script A to get names & index pairings securely
async function fetchFileListFromScriptA(code) {
  const response = await fetch(`${SCRIPT_A_URL}?code=${encodeURIComponent(code)}`);
  if (!response.ok) throw new Error("Script A network connectivity failed.");
  return await response.json();
}

// Communicates with Script B when a student explicitly clicks a specific file index row
async function fetchFileLinkFromScriptB(code, index) {
  ...
  if (!response.ok) throw new Error("Script B network connectivity failed.");
  return await response.json();
}
